...
angular.module('hrss.si.enterprise.directivesModule')
  //定义的时候注意开头小写
  .directive('sipubAttachUploadList', function () {
    return {
      restrict: 'E',
      templateUrl: 'modules/common/directives/attach-upload-list-directive/attach-upload-list-view.html',
      scope: {
      ...
      controller: 'SipubUploadAttachListClr'
    };
  })
 ...
 * 批量申报列表控制器
 */
  .controller('SipubUploadAttachListClr', ['ApplyResourceFactory','$scope', 'messageService', '$log', 'APPLY_APP_EVENT', 'AttachService',
    'messageBox', 'HRSS_APP_USER_DETAILS_CACHE','$interval',
    function (ApplyResourceFactory,$scope, messageService, $log, APPLY_APP_EVENT, AttachService, messageBox, HRSS_APP_USER_DETAILS_CACHE, $interval) {
        var ApplyResource = AttachService.crtResource($scope.applyType);
        $scope.applyTypeValue='材料批量上传_'+$scope.applyTypeValue;
      // 定时器
      var timer;
      // 刷新时间间隔
      var times = 2000;
      ...
      $scope.listColumnDefs = [
        { field: 'fileName', displayName: '文件名称',headerCellClass: 'ngCellCenter',cellClass: 'ngCellCenter', width: 200,cellFilter: 'fileUploadNameFilter'},
        ...
        { field: 'errCount', displayName: '错误数量',headerCellClass: 'ngCellCenter',cellClass: 'ngCellCenter', width: 80},
       ...
       * 响应报盘完成事件
       */
      $scope.$on('attachUploadComplete', function (event,result) {
        $log.debug('接收到attachUploadComplete事件');
        debugger
        //刷新清单
        $scope.getUploadList();
        $scope.rowCount = $scope.getRowCount();
